...
program			: MAINPROG Identifier SEMICOLON declarations subprogram_declarations compound_statement {  eval($6);}
					;
...
type		: standard_type                  {
      ...
      | ARRAY L_BRACKET Integer R_BRACKET OF standard_type   {
																					char* len = (char*)malloc(sizeof(char)* 10);
																				  sprintf(len, "%d", (int)$3);
																				  char * tmp = (char *)malloc(sizeof(char) * 5);
...
subprogram_declaration: subprogram_head declarations compound_statement {$1->func = $3;$1->value =0;}
                  ;
...
variable				: Identifier { char* temp = (char*)malloc(sizeof(char)*20);
											...
											$$ = temp_sym; }
         				| Identifier L_BRACKET expression R_BRACKET { char* temp = (char*)malloc(sizeof(char)*20);
                                                                  int index = (int)eval($3);
                                                                  strcpy(temp,$1);
                                                                  struct symbol* temp_sym= (struct symbol*)malloc(sizeof(struct symbol)*1);
                                                                  temp_sym = lookup(temp);
                                                                  ...
                                                                  while(index>0){
                                                                        if(temp_sym->arry)
                                                                        {
                                                                           temp_sym = temp_sym->arry;
                                                                        }
                                                                        else{ printf("no Declare\n");}
                                                                        index--;
                                                                  }
                                                                  $$ = temp_sym;
																					}
...
void newDeclareIdentifier(struct symlist * s, char *type){
   ...
   struct symbol * sym_temp;
   struct symlist * s_temp = (struct symliset*)malloc(sizeof(struct symlist));
   s_temp =s;
   ...
   if(type[0] == '0'){
      ...
      len = &type[2];
      arr_len = atoi(len);
      while (s_temp->sym != NULL){
         t = s_temp->sym;
         t->type = type[1];
         while(arr_len > 1){
            //printf("creat array\n");
            sym_temp = malloc(sizeof(struct symbol));
            t-> arry = sym_temp;
      ...
      while(k){
         if(k->arry){
            k-> arry = (struct symbol*)malloc(sizeof(struct symbol));
               printf("s->symbol : %s\n",k->arry->name);
               k = k->arry;

         }
         else{printf("nononoononononon\n");break;}
      }
   }
}
